...
class ExamView{	
	...
	function publishaction($bean, $event, $argument){
		...
		$html='<a id="'.$bean->id.'" value='.$bean->id.' class="button">PUBLISH</a><a id="'.$bean->id.'_32" href="index.php?module=te_Exam&action=editexam&id='.$bean->id.'" target="_blank" class="button">Edit</a><script>
			...
			$.ajax({
			  ...
			  success: function(msg){
				...
				if('.$status.'==1){
					...
					$("#'.$bean->id.'_32").css("background-color","#FFEBCD");
				}
			  }
		   });
		   } else {
