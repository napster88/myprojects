...
if($count>0){
	echo '<script> alert("File Successfully Uploaded");callPage(); function callPage(){ window.location.href="index.php?module=te_Exam_result&action=index"} </script>';
					...
					$sugarSmarty->assign("institutes",$this->get_all_institutes());
					$sugarSmarty->assign("ins_programs",$this->get_all_courses_with_ins());
...
function get_all_institutes(){
	...
	$institute_sql="SELECT ins.id AS id,ins.name as name FROM `te_in_institutes` AS ins WHERE ins.deleted=0";
	$institute_obj=$db->query($institute_sql);
	$resultArr = [];
	if($institute_obj->num_rows){
		while($row=$db->fetchByAssoc($institute_obj)){
			$resultArr[]=$row;
		}
	}
...
function get_all_courses_with_ins(){
	global $db;

	$course_sql="SELECT p.id AS pro_id,p.name AS pro_name,i.id AS ins_id,i.name AS ins_name FROM te_pr_programs AS p INNER JOIN te_in_institutes_te_pr_programs_1_c AS ipr ON ipr.te_in_institutes_te_pr_programs_1te_pr_programs_idb=p.id AND ipr.deleted=0 INNER JOIN te_in_institutes AS i ON i.id=ipr.te_in_institutes_te_pr_programs_1te_in_institutes_ida AND i.deleted=0 WHERE p.deleted=0";
	$course_obj=$db->query($course_sql);
	$resultArr = [];
	if($course_obj->num_rows){
		while($row=$db->fetchByAssoc($course_obj)){
			$resultArr[]=$row;
		}
	}
