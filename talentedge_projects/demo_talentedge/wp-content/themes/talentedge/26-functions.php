...
function coupon_code_check()
{
    ...
    if (isset($_POST['coupon']))
    {
        ...
        foreach ($coupons as $coupon)
        {
            ...
            {
                $msg = 'Invalid Scholarship ID. Enter a valid ID or talk to our counsellors';
            }
        }
        if ($d_pid != '')
        {
...
function get_product_price($product_id, $currency)
{
    if ($currency == 'INR')
    {
    ...
    if ($loginLinkedinCheck[0] == 'yes')
    {
        ...
        if (have_rows('linkedin_designation', 'option')):
            while (have_rows('linkedin_designation', 'option')) : the_row();
                ...
                ?>
                        <option value='<?php echo $c_designation; ?>' <?php if ($suggDesignation[0] == $c_designation) echo "selected"; ?>><?php echo get_sub_field('designation'); ?></option>
                <?php
            endwhile;
        ...
        if (have_rows('linkedin_industry', 'option')):
            ...
            <input class="submit_sugg" type="submit" name="submitCoursevalue" value="Submit">   
                                ...
                                <?php get_popularcourses_by_userinput($userId); ?>
                            <?php }
                            else
                            {
                                ?>
...
function get_Installment_price3($product_id, $stax = 0)
{
    ...
    $course_price_inr2   = get_product_price($product_id, 'INR');
    $course_price_inr    = $course_price_inr2[0];
    $course_price_format = number_format($course_price_inr);
    //$course_price_usd = get_post_meta( $product_id, '_outside-india_regular_price', true);

    $course_price_usd2       = get_product_price($product_id, 'USD');
    $course_price_usd        = $course_price_usd2[0];
    ...
    if (!in_array($product_id, $product_p))
    {
        ...
        ?>
        <style>
            #course_coupon{height: 35px;margin: 0px 10px 0px 0px;padding:5px 15px 5px 5px;text-transform: uppercase;border-bottom: 1px solid #f2f2f2 !important;
                           border: none;}
        ...
        <input type="hidden" name="course_id" class="course_id" id="course_actual_price_inr" value="<?php echo $course_price_inr2; ?>"/>
        <input type="hidden" name="course_id" class="course_id" id="course_actual_price_usd" value="<?php echo $course_price_usd2; ?>"/>
        ...
        <?php } ?>
            ...
            <input type="hidden" name="coursetype" id="coursetype" value="1"/> 
        ...
        <div class="course_details col-md-4">
            <div class="paymentdiv">
                <div class="totalprice first">
                    ...
                    <div class="fright">
                        <span class="c_currency pricelabel"><?php echo $currencySymbol; ?></span><span class="full_actualprice pricelabel"><?php echo $course_price_format; ?></span>
                    </div>
                </div>
    ...
    else
    {
        ...
        $order_ids = get_all_ordersby_pname($product_id);
        ...
        $regularPrice = get_product_price($product_id, $order_currency);
            ...
            <div class="course_details_div">
                ...
                <table class="table">
                    <thead>
                        ...
                        <tr><th>Balance Amount</th><th class="pamt"><span class="c_currency"><?php echo $currencySymbol; ?></span><?php echo number_format($pending_amount); ?></th></tr>
                    </thead>
                    </tbody>
                </table>
            </div>
        ...
        ?>
                <div class="finalamt totalprice">
                    ...
                    <div class="fright">
                        <span class="c_currency pricelabel"><?php echo $currencySymbol; ?></span><span class="final_amt pricelabel"><?php echo number_format($new_amount); ?></span>
                    </div>
                </div>
            </div>
        </div>
...
function get_installment_data($product_id, $selectedCurrency)
{
    $currencySymbol = get_woocommerce_currency_symbol($selectedCurrency);
    ?>
...
add_filter('acf/load_field/key=field_57ff5da03b1f6', 'field_57d64dfaa45ef');
...
function get_all_ordersby_pname($prod_id)
{
    ...
    //echo ">>>>>". $p_title; 
    $order_ids = $wpdb->get_results("select p.ID as order_id,p.post_date, p.post_status, max( CASE WHEN pm.meta_key = '_billing_email' and p.ID = pm.post_id THEN pm.meta_value END ) as billing_email, max( CASE WHEN pm.meta_key = 'payment_type' and p.ID = pm.post_id THEN pm.meta_value END ) as payment_type, max( CASE WHEN pm.meta_key = '_customer_user' and p.ID = pm.post_id THEN pm.meta_value END ) as customer_user, max( CASE WHEN pm.meta_key = '_order_total' and p.ID = pm.post_id THEN pm.meta_value END ) as order_total, max( CASE WHEN pm.meta_key = '_order_tax' and p.ID = pm.post_id THEN pm.meta_value END ) as order_tax, max( CASE WHEN pm.meta_key = '_order_currency' and p.ID = pm.post_id THEN pm.meta_value END ) as order_currency,
 ( select group_concat( order_item_name separator '|' ) from te_woocommerce_order_items where order_id = p.ID ) as order_items from te_posts p join te_postmeta pm on p.ID = pm.post_id join te_woocommerce_order_items oi on p.ID = oi.order_id where post_type = 'shop_order' and oi.order_item_name = '" . $postitle . "' group by p.ID");
...
{
    //    add_menu_page('My Page Title', 'My Menu Title', 'manage_options', 'my-page-slug', 'my_function');
    add_menu_page('Reports download', 'Reports download', 'read', 'order-report-download', 'download_order_report', '', '4');
}
...
{
    add_menu_page('Lead Report', 'Lead Report', 'read', 'download_lead_report', 'download_lead_report', '', '4');
}
        ...
        <form method="POST" action="#">
            ...
            <input type="submit" id="export-lead" class="button" value="Export" name="export">
                        ...
                        if (isset($start_date))
                        {
                            ...
                            /* Article Detail */
                            $form_id     = 22;
                            $entryform22 = GFAPI::get_entries($form_id, $search_criteria, '', $paging);

                            $arraynew_f22 = array();

                            foreach ($entryform22 as $entryform)
                            {

                                $arraynew_f22[] = $arrayName      = array('formid'          => $entryform['form_id'], 'formname'        => 'Article Detail', 'name'            => $entryform['1'], 'email'           => $entryform['13'], 'phone'           => $entryform['3'], 'utm-source'      => $entryform['4'],
                                    'utm-medium'      => $entryform['5'], 'utm-campaign'    => $entryform['6'], 'utm-term'        => $entryform['7'], 'course-name'     => $entryform['15'], 'inst-name'       => $entryform['16'], 'city'            => $entryform['10'], 'company'         => $entryform['11'], 'functional-area' => $entryform['12'], 'education'       => '', 'work-ex'         => '', 'date'            => $entryform['date_created'], 'source_url'      => $entryform['source_url'], 'leadid'          => $entryform['id']);
                            }

                            /* Article Detail Bottom */
                            $form_id     = 23;
                            $entryform23 = GFAPI::get_entries($form_id, $search_criteria, '', $paging);

                            foreach ($entryform23 as $entryform)
                            {
                                $arraynew_f22[] = $arrayName      = array('formid'          => $entryform['form_id'], 'formname'        => 'Article Detail Bottom', 'name'            => $entryform['1'], 'email'           => $entryform['13'], 'phone'           => $entryform['3'], 'utm-source'      => $entryform['4'],
                                    'utm-medium'      => $entryform['5'], 'utm-campaign'    => $entryform['6'], 'utm-term'        => $entryform['7'], 'course-name'     => $entryform['15'], 'inst-name'       => $entryform['16'], 'city'            => $entryform['10'], 'company'         => $entryform['11'], 'functional-area' => $entryform['12'], 'education'       => '', 'work-ex'         => '', 'date'            => $entryform['date_created'], 'source_url'      => $entryform['source_url'], 'leadid'          => $entryform['id']);
                            }
                            ...
                            $form_id     = 24;
                            $entryform24 = GFAPI::get_entries($form_id, $search_criteria, '', $paging);

                            foreach ($entryform24 as $entryform)
                            {
                                $arraynew_f22[] = $arrayName      = array('formid'          => $entryform['form_id'], 'formname'        => 'Article Detail Mobile', 'name'            => $entryform['1'], 'email'           => $entryform['13'], 'phone'           => $entryform['3'], 'utm-source'      => $entryform['4'],
                                    'utm-medium'      => $entryform['5'], 'utm-campaign'    => $entryform['6'], 'utm-term'        => $entryform['7'], 'course-name'     => $entryform['15'], 'inst-name'       => $entryform['16'], 'city'            => $entryform['10'], 'company'         => $entryform['11'], 'functional-area' => $entryform['12'], 'education'       => $entryform['8'], 'work-ex'         => $entryform['9'], 'date'            => $entryform['date_created'], 'source_url'      => $entryform['source_url'], 'leadid'          => $entryform['id']);
                            }
                            ...
                            $form_id    = 7;
                            $entryform7 = GFAPI::get_entries($form_id, $search_criteria, '', $paging);

                            foreach ($entryform7 as $entryform)
                            {
                                $arraynew_f22[] = $arrayName      = array('formid'          => $entryform['form_id'], 'formname'        => 'Landing Template', 'name'            => $entryform['1'], 'email'           => $entryform['13'], 'phone'           => $entryform['3'], 'utm-source'      => $entryform['4'],
                                    'utm-medium'      => $entryform['5'], 'utm-campaign'    => $entryform['6'], 'utm-term'        => $entryform['7'], 'course-name'     => $entryform['15'], 'inst-name'       => $entryform['16'], 'city'            => $entryform['10'], 'company'         => $entryform['11'], 'functional-area' => $entryform['12'], 'education'       => $entryform['8'], 'work-ex'         => $entryform['9'], 'date'            => $entryform['date_created'], 'source_url'      => $entryform['source_url'], 'leadid'          => $entryform['id']);
                            }

                            /* Landing Template Mobile */

                            $form_id     = 12;
                            $entryform12 = GFAPI::get_entries($form_id, $search_criteria, '', $paging);

                            foreach ($entryform12 as $entryform)
                            {
                                $arraynew_f22[] = $arrayName      = array('formid'          => $entryform['form_id'], 'formname'        => 'Landing Template Mobile', 'name'            => $entryform['1'], 'email'           => $entryform['13'], 'phone'           => $entryform['3'], 'utm-source'      => $entryform['4'],
                                    'utm-medium'      => $entryform['5'], 'utm-campaign'    => $entryform['6'], 'utm-term'        => $entryform['7'], 'course-name'     => $entryform['15'], 'inst-name'       => $entryform['16'], 'city'            => $entryform['10'], 'company'         => $entryform['11'], 'functional-area' => $entryform['12'], 'education'       => $entryform['8'], 'work-ex'         => $entryform['9'], 'date'            => $entryform['date_created'], 'source_url'      => $entryform['source_url'], 'leadid'          => $entryform['id']);
                            }

                            /* Multistep Footer form */

                            $form_id    = 1;
                            $entryform1 = GFAPI::get_entries($form_id, $search_criteria, '', $paging);

                            foreach ($entryform1 as $entryform)
                            {
                                $arraynew_f22[] = $arrayName      = array('formid'          => $entryform['form_id'], 'formname'        => 'Multistep Footer form', 'name'            => $entryform['3'], 'email'           => $entryform['9'], 'phone'           => $entryform['1'], 'utm-source'      => $entryform['10'],
                                    'utm-medium'      => $entryform['11'], 'utm-campaign'    => $entryform['12'], 'utm-term'        => $entryform['13'], 'course-name'     => $entryform['15'], 'inst-name'       => $entryform['14'], 'city'            => $entryform['6'], 'company'         => $entryform['7'], 'functional-area' => $entryform['8'], 'education'       => '', 'work-ex'         => '', 'date'            => $entryform['date_created'], 'source_url'      => $entryform['source_url'], 'leadid'          => $entryform['id']);
                            }
                            ...
                            $form_id    = 4;
                            $entryform4 = GFAPI::get_entries($form_id, $search_criteria, '', $paging);


                            foreach ($entryform4 as $entryform)
                            {
                                $arraynew_f22[] = $arrayName      = array('formid'          => $entryform['form_id'], 'formname'        => 'Call back enquiry', 'name'            => $entryform['1'], 'email'           => $entryform['8'], 'phone'           => $entryform['3'], 'utm-source'      => $entryform['9'],
                                    'utm-medium'      => $entryform['10'], 'utm-campaign'    => '', 'utm-term'        => '', 'course-name'     => '', 'inst-name'       => '', 'city'            => $entryform['5'], 'company'         => $entryform['6'], 'functional-area' => $entryform['7'], 'education'       => '', 'work-ex'         => '', 'date'            => $entryform['date_created'], 'source_url'      => $entryform['source_url'], 'leadid'          => $entryform['id']);
                            }

                            /* Course Detail */

                            $form_id    = 5;
                            $entryform5 = GFAPI::get_entries($form_id, $search_criteria, '', $paging);

                            foreach ($entryform5 as $entryform)
                            {
                                $arraynew_f22[] = $arrayName      = array('formid'          => $entryform['form_id'], 'formname'        => 'Course Detail', 'name'            => $entryform['1'], 'email'           => $entryform['11'], 'phone'           => $entryform['3'], 'utm-source'      => $entryform['12'],
                                    'utm-medium'      => $entryform['13'], 'utm-campaign'    => '', 'utm-term'        => $entryform['15'], 'course-name'     => $entryform['6'], 'inst-name'       => $entryform['14'], 'city'            => $entryform['8'], 'company'         => $entryform['9'], 'functional-area' => $entryform['10'], 'education'       => '', 'work-ex'         => '', 'date'            => $entryform['date_created'], 'source_url'      => $entryform['source_url'], 'leadid'          => $entryform['id']);
                            }

                            /* Course Detail Mobile */

                            $form_id     = 10;
                            $entryform10 = GFAPI::get_entries($form_id, $search_criteria, '', $paging);

                            foreach ($entryform10 as $entryform)
                            {
                                $arraynew_f22[] = $arrayName      = array('formid'          => $entryform['form_id'], 'formname'        => 'Course Detail Mobile', 'name'            => $entryform['1'], 'email'           => $entryform['15'], 'phone'           => $entryform['3'], 'utm-source'      => $entryform['11'],
                                    'utm-medium'      => $entryform['12'], 'utm-campaign'    => '', 'utm-term'        => $entryform['14'], 'course-name'     => $entryform['6'], 'inst-name'       => $entryform['13'], 'city'            => $entryform['8'], 'company'         => $entryform['9'], 'functional-area' => $entryform['10'], 'education'       => '', 'work-ex'         => '', 'date'            => $entryform['date_created'], 'source_url'      => $entryform['source_url'], 'leadid'          => $entryform['id']);
                            }
                            ...
                            {
                                $arraynew_f22[] = $arrayName      = array('formid'          => $entryform['form_id'], 'formname'        => 'Category Detail', 'name'            => $entryform['1'], 'email'           => $entryform['8'], 'phone'           => $entryform['3'], 'utm-source'      => $entryform['9'],
                                    'utm-medium'      => $entryform['10'], 'utm-campaign'    => '', 'utm-term'        => '', 'course-name'     => '', 'inst-name'       => '', 'city'            => $entryform['5'], 'company'         => $entryform['6'], 'functional-area' => $entryform['7'], 'education'       => '', 'work-ex'         => '', 'date'            => $entryform['date_created'], 'source_url'      => $entryform['source_url'], 'leadid'          => $entryform['id']);
                            }
                            ...
                            {
                                $arraynew_f22[] = $arrayName      = array('formid'          => $entryform['form_id'], 'formname'        => 'Category Detail Mobile', 'name'            => $entryform['1'], 'email'           => $entryform['8'], 'phone'           => $entryform['3'], 'utm-source'      => $entryform['9'],
                                    'utm-medium'      => $entryform['10'], 'utm-campaign'    => '', 'utm-term'        => '', 'course-name'     => '', 'inst-name'       => '', 'city'            => $entryform['5'], 'company'         => $entryform['6'], 'functional-area' => $entryform['7'], 'education'       => '', 'work-ex'         => '', 'date'            => $entryform['date_created'], 'source_url'      => $entryform['source_url'], 'leadid'          => $entryform['id']);
                            }
                            ...
                            $form_id    = 2;
                            $entryform2 = GFAPI::get_entries($form_id, $search_criteria, '', $paging);

                            foreach ($entryform2 as $entryform)
                            {
                                $arraynew_f22[] = $arrayName      = array('formid'          => $entryform['form_id'], 'formname'        => 'Insitute Detail', 'name'            => $entryform['1'], 'email'           => $entryform['8'], 'phone'           => $entryform['3'], 'utm-source'      => $entryform['9'],
                                    'utm-medium'      => $entryform['10'], 'utm-campaign'    => '', 'utm-term'        => '', 'course-name'     => '', 'inst-name'       => '', 'city'            => $entryform['5'], 'company'         => $entryform['6'], 'functional-area' => $entryform['7'], 'education'       => '', 'work-ex'         => '', 'date'            => $entryform['date_created'], 'source_url'      => $entryform['source_url'], 'leadid'          => $entryform['id']);
                            }
                            ...
                            $form_id     = 18;
                            $entryform18 = GFAPI::get_entries($form_id, $search_criteria, '', $paging);

                            foreach ($entryform18 as $entryform)
                            {
                                $arraynew_f22[] = $arrayName      = array('formid'          => $entryform['form_id'], 'formname'        => 'Request For a Demo', 'name'            => $entryform['1'], 'email'           => $entryform['2'], 'phone'           => $entryform['4'], 'utm-source'      => $entryform['6'],
                                    'utm-medium'      => $entryform['7'], 'utm-campaign'    => '', 'utm-term'        => '', 'course-name'     => '', 'inst-name'       => '', 'city'            => '', 'company'         => $entryform['3'], 'functional-area' => '', 'education'       => '', 'work-ex'         => '', 'date'            => $entryform['date_created'], 'source_url'      => $entryform['source_url'], 'leadid'          => $entryform['id']);
                            }
                            ...
                            ob_end_clean();
                            $output = fopen('php://output', 'w');
                            fputcsv($output, array('Formid', 'Form Name', 'Name', 'Email', 'Phone',
                                'Utm Source', 'Utm Medium', 'Utm Campaign', 'Utm Term', 'Course Name', 'Institute Name', 'City', 'Company',
                                'Functional Area', 'Education', 'Work Ex', 'Date', 'Source Url', 'Lead id'));
...
<div class="card">
    ...
    <h2><a href="<?php echo $course_link; ?>"><?php echo $course_short_name; ?></a></h2>
</div>
<ul>
<?php
$k = 1;
...
<div class="btn-te"><a class="redir_btn-a" href="<?php echo $course_link; ?>" title="<?php echo $course_shortname; ?>">View Details</a></div>
</div>
</div>
</li>
...
function overrideTax($taxes, $price, $rates, $price_includes_tax, $suppress_rounding)
{
    ...
    //print_r(WC_Tax::get_tax_location()); 
    $country = '';
    $state   = '';
    ...
    if ($id == 'customer_completed_order')
    {
        ...
        if (!get_post_meta($object->id, '_bewpi_invoice_number', true))
        {
            ...
            else
            {
                ...
                $headers[] = 'Bcc: garima.gupta@talentedge.in, deepak.sharma@talentedge.in,pramod.singh@talentedge.in, nitin.bhardwaj@talentedge.in, aditi.kochhar@talentedge.in, eleazer.rohit@talentedge.in, surendra.saini@talentedge.in, tuli.sengupta@talentedge.in, deepak.yadav@talentedge.in, swapna.madiwala@talentedge.in, cbo.consumer@talentedge.in, vidhu.singh@talentedge.in, swati.belekar@talentedge.in,website.payments@talentedge.in,karan.bhatia@talentedge.in,shilpi.sharma@talentedge.in,amit.arora@talentedge.in,payments.te@talentedge.in';
