		...
		switch($template) {
			...
			case 'change':
				...
				if (empty($output['error'])) {
					...
					if (!$users[0]) {
						$output['error']['secretkey'] = __('The secret key is invalid or.','userpro');
					} else {
			...
			case 'reset':
				...
				if (!$username_or_email){
					$output['error']['username_or_email'] = __('You should provide your email.','userpro');
				} else {
			...
			case 'login':
				...
				if ( is_wp_error($user) ) {
					...
					$output['error']['username_or_email'] = __('User not found','userpro');
		...
		switch($ajaxcheck) {
			...
			case 'email_exists':
				if (!is_email($input_value)) {
					...
					$output['error'] = __('Email is already registered','userpro');
				}
				break;
			...
			case 'validatesecretkey':
				if (strlen($input_value) != 20) {
					...
					jQuery(document).ready(function($) {
						 var resetPassword = getParameterByName('a');
						 if(resetPassword == 'reset'){
						 	console.log("aaaaaaaaa");
						 	$(.logindiv .userpro-body).after("<div class='userpro-warning customWarning' style='opacity: 1; top: 0px;'>You should provide your email.</div>")
						}
					});
					</script>

				<?php 		$output['error'] = __('The secret key is invalid or expir.','userpro');
					}
				}
				break;
