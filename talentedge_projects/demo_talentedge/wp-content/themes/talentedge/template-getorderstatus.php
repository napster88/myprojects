    ...
    while ($query->have_posts()) : $query->the_post();
            ...
            foreach ($arraya as $value)
            {
                foreach ($value as $val1)
                {
                    ...
                    if ($val1->status == "success")
                    {
                        ...
                        fwrite($fp, "[" . date("F d, Y h:i:s A") . "] Payment completed Order: $post_id with status: completed, by PayU).");
                        fclose($fp);
                        
                        $crmOrderId = get_post_meta($post_id, 'crm_orderid', true);
                        if(!$crmOrderId){
                            crm_create_payments($post_id);
                         }
                    }
                    if ($val1->status == "failure")
                    {
                        ...
                        $fp = fopen('API_fail_payment_log_.txt', 'a');
                        fwrite($fp, "[" . date("F d, Y h:i:s A") . "] Payment failure Order: $post_id with status: fail, by PayU).");
                        fclose($fp);
                    }
                }
            }
        ...
        if ($payment_method == 'atom')
        {
            ...
            foreach ($array1 as $atomdata)
            {
                ...
                if ($atomdata['VERIFIED'] == 'SUCCESS')
                {
                    ...
                    $updatedOrder[] = $post_id;
                    $fp             = fopen('API_completed_payment_log_.txt', 'a');
                    fwrite($fp, "[" . date("F d, Y h:i:s A") . "] Payment completed Order: $post_id with status: completed, by atom).");
                    fclose($fp);
                ...
                if ($atomdata['VERIFIED'] == 'FAILED')
                {
                   ...
                   $fp = fopen('API_fail_payment_log_.txt', 'a');
                   fwrite($fp, "[" . date("F d, Y h:i:s A") . "] Payment failure Order: $post_id with status: fail, by atom).");
                   fclose($fp);
                }
            }
        }
        ...
        if ($payment_method == 'paytm')
        {
            ...
            if ($finalResults->STATUS == 'TXN_SUCCESS')
            {
                ...
                $fp = fopen('API_completed_payment_log_.txt', 'a');
                fwrite($fp, "[" . date("F d, Y h:i:s A") . "] Payment completed Order: $post_id with status: completed, by paytm).");
                fclose($fp);
                $crmOrderId = get_post_meta($post_id, 'crm_orderid', true);
                        if(!$crmOrderId){
                            crm_create_payments($post_id);
            ...
            if ($finalResults->STATUS == 'TXN_FAILURE')
            {
                ...
                $updatedOrder[] = $post_id;
                 ...
                 $fp = fopen('API_fail_payment_log_.txt', 'a');
                   fwrite($fp, "[" . date("F d, Y h:i:s A") . "] Payment failure Order: $post_id with status: fail, by paytm).");
                   fclose($fp);
            }
        }
