...
class Branchs extends CI_Controller {
   ...
   public function index()
   {
      ...
      if(isset($_POST) && count($_POST) > 0 && !isset($_POST['lang_submit']))
      {
        ...
        if($checks !== FALSE)
        {
           ...
           {
             if(! ($this->data->delete("support",$check)))
             {
      ...
      $data['data']        = $this->data->get("branchs",FALSE,array("id >"=>0),FALSE,TRUE,array("id"=>"asc")) ;
     ...
     $this->load->view('administrator/pages/branchs_table',$data);
   ...
   public function add()
   {
      ...
      {
        $data['data']          = $this->data->get("branchs",$data['langId'],array("id"=>$this->uri->segment(4))) ;
      ...
      if(isset($_POST) && count($_POST) > 0 && !isset($_POST['lang_submit']))
      {
       ...
       //	'mobile'=>"رقم الموبايل",
    	...
    	}else
    	{
    		if($this->data->save())
            {
               ...
               $data['data']          = $this->data->get("branchs",FALSE,array("id"=>$this->uri->segment(4))) ;
            }
            else
            {
       ...
       $this->load->view('administrator/global/topbar');
       $this->load->view('administrator/global/menu');
       $this->load->view('administrator/pages/branchs_form',$data);
       $this->load->view('administrator/global/footer');
   }
//*****************************************************************
   public function delete($slug)
	{
		if($slug > 0)
		{
		  	$this->data->delete("branchs",$slug);
		}
    	redirect('administrator/branchs', 'refresh');
