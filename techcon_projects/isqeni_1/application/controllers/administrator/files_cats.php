...
class Files_cats extends CI_Controller {
   ...
   public function index()
   {
      ...
      if(isset($_POST) && count($_POST) > 0 && !isset($_POST['lang_submit']))
      {
        ...
        if($checks !== FALSE)
        {
           ...
           {
             $slid = $this->data->get("files_cats",FALSE,array("id"=>$check)) ;
		     $path = "./download/files_cats/".$slid['photo'];
             if(is_file($path)){unlink($path) ; }
             if(! ($this->data->delete("files_cats",$check)))
             {
      ...
      $data['data']        = $this->data->get("files_cats",$data['langId']) ;
     ...
     $this->load->view('administrator/pages/files_cats_table',$data);
   ...
   public function add()
   {
      ...
      {
        $data['data']          = $this->data->get("files_cats",$data['langId'],array("id"=>$this->uri->segment(4))) ;
      ...
      if(isset($_POST) && count($_POST) > 0 && !isset($_POST['lang_submit']))
      {
       ...
       $this->load->view('administrator/global/topbar');
       $this->load->view('administrator/global/menu');
       $this->load->view('administrator/pages/files_cats_form',$data);
       $this->load->view('administrator/global/footer');
   }
   ...
   public function delete($slug)
	{
		if($slug > 0)
		{
            $slid = $this->data->get("files_cats",FALSE,array("id"=>$slug)) ;
		     $path = "./download/files_cats/".$slid['photo'];

             if(is_file($path)){unlink($path) ; }
		  	$this->data->delete("files_cats",$slug);
		}
    	redirect('administrator/files_cats', 'refresh');
