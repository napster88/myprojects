...
class AOR_ReportsViewCertificate extends SugarView {
    ...
    $leadsData=array();
		...
		if(isset($_POST['export']) && $_POST['export']=="Export"){
			$data="Student,Batch,Email,Phone,Address,Certificate,Status\n";
			$file = "certificate_report";
			$filename = $file . "_" . date ( "Y-m-d");
			foreach($councelorList as $key=>$councelor){
				if($councelor['certificate_sent']==1){
					$status='Yes';
				}
				else if($councelor['certificate_sent']==2){
					$status='Yes';
				}
				else{
					$status='NA';
				}
				$data.= "\"" . $councelor['student'] . "\",\"" . $councelor['batch'] . "\",\"". $councelor['email']. "\",\"". $councelor['mobile']. "\",\"". $councelor['completion_certificate_address']. "\",\"". $councelor['eligible_for_certificate']. "\",\"". $status. "\"\n";
			}
			ob_end_clean();
