...
{literal}
<style>.footable-last-column{display:none}

#delete_listview_top,#select_actions_disabled_top,#select_actions_disabled_bottom,#delete_listview_bottom,.checkbox,.paginationActionButtons{display:none}
</style>
{/literal}
...
{if $hideTable == false}
	...
	{capture assign=url}index.php?module={$linkModule}&offset={$offset}&stamp={$pageData.stamp}&return_module={$linkModule}&action={$action}&record={$record}{/capture}
				...
				{assign var='statusbtn' value=$rowData.ID|getStatus}
				{assign var='statusbtnSubmitter' value=$rowData.ID|getStatussubmitter}			
				{assign var='getCanCancel' value=$rowData.ID|checkParentApproved}
					
				  {if $statusbtnSubmitter==2}
					 
								  <a title='{$editLinkString}' id="edit-{$rowData.ID}" class="button"
                           href="index.php?module=te_expense_vendor&offset={$offset}&stamp={$pageData.stamp}&return_module={$linkModule}&action={$action}&record={$rowData.ID}"
                                >NO ACTION</a>
				  {else}
					 {if $statusbtnSubmitter==-1 || $statusbtnSubmitter==3}
						NO ACTION
					 {else}
					  {if $statusbtn==-2 && $rowData.STATUS!='Approved'}	
					   {if $getCanCancel==1}
					     <a title='{$editLinkString}' id="edit-{$rowData.ID}" class="button"
                           href="index.php?module=te_expense_vendor&offset={$offset}&stamp={$pageData.stamp}&return_module={$linkModule}&action={$action}&record={$rowData.ID}"
                                >
