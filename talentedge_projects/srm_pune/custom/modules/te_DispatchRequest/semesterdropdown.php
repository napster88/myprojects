...
/* For Dispatch ietem request dropdown */
if(isset($_REQUEST['subjectIdsem']) && $_REQUEST['subjectIdsem']!=""){
	
	global $db;
	
	$status=$_REQUEST['subjectIdsem'];
	
	$batchSqlS="SELECT sub.id, sub.name FROM te_subjects_master AS sub INNER JOIN te_subjects_master_te_te_semester_1_c AS subrel ON sub.id = subrel.te_subjects_master_te_te_semester_1te_subjects_master_ida WHERE subrel.te_subjects_master_te_te_semester_1te_te_semester_idb='".$status."' AND sub.deleted=0";
	$batchObje =$db->query($batchSqlS);
	$SEmOptions=array('status'=>'error','id'=>'');
	while($rows =$db->fetchByAssoc($batchObje)){ 
		$SEmOptions['res'][]=$rows;
