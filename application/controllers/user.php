...
class User extends CI_Controller {
	...
	public function adduser()
	{
		...
		else
		{
			...
			$role = 2;
			$status = 1;
			$result = $this->user_model->adduser($emailid,$pass,$cpass,$username,$first_name,$last_name,$role,$status,$path);
			//print_r($result);die();
			if($result != 1){
	...
	public function updateuser($id){
		...
		$result = $this->user_model->update_user($first_name,$last_name,$role,$id);
		//print_r($result);die();
		if($result <= 0){
			$this->session->set_flashdata('flashdata', 'Something went wrong, please try again .');
