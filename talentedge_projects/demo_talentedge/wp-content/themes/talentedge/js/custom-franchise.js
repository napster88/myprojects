...
function gallResize(){
	$('.overlayBox').each(function(){
		...
		winHeight = $(window).height();
		var formWidth = $(this).find('.overlayForm').width();
		$(this).find('.largeImg li img').css({'max-width':(winWidth-formWidth)-100,'max-height':winHeight-100});
		var ImageWidth = $(this).find('.ActiveImg').width();
		var ImageHeight = $(this).find('.ActiveImg').height();
		$(this).find('.overlayImage').css({'width':ImageWidth,'height':ImageHeight});
...
function wResize(){
	winWidth = $(window).width();
	winHeight = $(window).height();
	$('.blackOverlay').css({'width':winWidth,'height':winHeight});
}
...
$(function(){
	wResize();
	gallResize();
	var formCon = $('#formCon').html();
	$('.overlayFormData').html(formCon);
	$('.showOverlay, .overlay-button').on('click',function(){
		$('.ImageGall').show();
		var getUrl = $(this).closest('.gall-parent').find('.showOverlay').attr('src');
  		laggeImg = getUrl.slice(0, -4)+'_big.jpg';
  		$('.ImageGall').find('.overlayImage img').attr('src',laggeImg);
		return false;
	});
	$('.overlayClose, .overlayCon').on('click',function(){
		closePopUp();
	});
	...
	function closePopUp(){
		$('.overlayCon').hide();
		$('.overlayBox').each(function() {
			$(this).find('.largeImg li').fadeOut(500).removeClass('ActiveImg');
			$(this).find('.largeImg li:first').fadeIn(500).addClass('ActiveImg');
			$(this).find('.thumbImg li').removeClass('thumbActive');
			$(this).find('.thumbImg li:first').addClass('thumbActive');			
		})
	}
	$('.enquiry-button').on('click',function(){
		var getClass = $(this).attr('href');
		$('.blackOverlay').show();
		$('.'+getClass).show();
		return false;
	});
	$('.GallOverlay, .gallButton').on('click',function(){
		$(this).closest('.overlay-parent').find('.overlayCon').show();
		gallResize();
		return false;
	});

	$('.overlayBox').each(function() {
		var imageLent = $(this).find('.largeImg li').length
        var ThumbImg = $(this).find('.thumbImg li');
		var LargeImg = $(this).find('.largeImg li');
		var rightSlide = $(this).find('.rightArrow');
		var leftSlide = $(this).find('.leftArrow');
		$(ThumbImg).click(function(){
			var crtImage = $(this).index();
			$(this).parent().find('li').removeClass('thumbActive');
			$(this).addClass('thumbActive');
			$(LargeImg).not($(LargeImg).eq(crtImage)).fadeOut(500).removeClass('ActiveImg');
			$(LargeImg).eq(crtImage).fadeIn(500).addClass('ActiveImg');
			gallResize();
		});
		$(rightSlide).click(function(){
			crtSlide = $(this).parent().find('.largeImg .ActiveImg');
			nextSlide = (crtSlide.next().length)? $(crtSlide).next() : $(this).next().find('li:first')
			$(crtSlide).fadeOut(500).removeClass('ActiveImg');
			$(nextSlide).fadeIn(500).addClass('ActiveImg');
			var thumbIndex = crtSlide.index();
			crtThumb = 1+thumbIndex;
			if(crtThumb==(imageLent)){
				crtThumb = 0;
			}
			$(ThumbImg).removeClass('thumbActive');
			$(ThumbImg).eq(crtThumb).addClass('thumbActive');
			gallResize();
		});
		$(leftSlide).click(function(){
			crtSlide = $(this).parent().find('.largeImg .ActiveImg');
			prevSlide = (crtSlide.prev().length)? $(crtSlide).prev() : $(this).parent().find('.largeImg li:last')
			$(crtSlide).fadeOut(500).removeClass('ActiveImg');
			$(prevSlide).fadeIn(500).addClass('ActiveImg');
			var thumbIndex = crtSlide.index();
			crtThumb = thumbIndex-1;
			if(crtThumb==(imageLent)){
				crtThumb = 0;
			}
			$(ThumbImg).removeClass('thumbActive');
