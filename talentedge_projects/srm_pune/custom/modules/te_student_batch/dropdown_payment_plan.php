...
if(isset($_REQUEST['batchIdsem']) && $_REQUEST['batchIdsem']!=""){
	...
	$batchSqlS="SELECT sem.name,sem.id from te_te_semester sem INNER JOIN te_pr_programs_te_te_semester_1_c prsem ON prsem.te_pr_programs_te_te_semester_1te_te_semester_idb =sem.id INNER JOIN te_pr_programs_te_ba_batch_1_c AS pbt ON pbt.te_pr_programs_te_ba_batch_1te_pr_programs_ida=prsem.te_pr_programs_te_te_semester_1te_pr_programs_ida WHERE sem.deleted=0 AND prsem.deleted=0 AND pbt.te_pr_programs_te_ba_batch_1te_ba_batch_idb='".$status."' ORDER BY order_name ASC";
...
if(isset($_REQUEST['batchIdenroll']) && $_REQUEST['batchIdenroll']!=""){
	$status=$_REQUEST['batchIdenroll'];
	$batchSqlS="SELECT `duration` from te_ba_batch where id='".$status."'";
	$batchObje =$db->query($batchSqlS);
	$SEmOptions=array('status'=>'error','id'=>'');
	while($rows =$db->fetchByAssoc($batchObje)){ 
		$SEmOptions['res'][]=$rows;
