...
class TargetVsActualBatchSizeDashlet extends Dashlet{ 
    protected $TargetVsActualBatchSizeDashlet = '';
    ...
    {        	
		...
		if(!empty($def['TargetVsActualBatchSizeDashlet'])){
            $this->TargetVsActualBatchSizeDashlet = $def['TargetVsActualBatchSizeDashlet'];
        }else{			       
			$this->TargetVsActualBatchSizeDashlet=10;
		} 		
    ...
    public function display() 
    {
        ...
        $ss->assign('TargetVsActualBatchSizeDashlet', $this->TargetVsActualBatchSizeDashlet);
    ...
    public function displayOptions(){
		...
		if($this->isAutoRefreshable()) {
		 ...
		 return parent::displayOptions() . $ss->fetch('custom/modules/te_ba_Batch/Dashlets/TargetVsActualBatchSizeDashlet/TargetVsActualBatchSizeDashletOptions.tpl');
    }  
	/**
    ...
    public function saveOptions(array $req){
        ...
        $options['TargetVsActualBatchSizeDashlet'] = $req['TargetVsActualBatchSizeDashlet'];
    ...
    protected function getCurrentBenchOutput()
    {
		...
		$leadQuery = "SELECT b.id,b.name,b.batch_size,GROUP_CONCAT(DISTINCT(te_utm.name))UTM FROM te_ba_batch b LEFT JOIN te_utm ON te_utm.te_ba_batch_id_c=b.id WHERE b.deleted = 0 AND b.batch_status = 'enrollment_in_progress'AND te_utm.utm_status='Live' GROUP BY b.id limit 0,".$this->TargetVsActualBatchSizeDashlet;
