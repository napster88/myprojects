...
<?php }?>
<?php if($active == 'user' || $active == 'Account' || $active == 'vehicle' || $active == 'page' || $active == 'service' || $active == 'plan' || $active == 'transaction' || $active == 'order' || $active == 'account_social'|| $active == 'Payment'|| $active == 'ups'|| $active == 'Subscription'|| $active == 'referal') {?>
...
<script>
	...
	$('.delete-alert').on('click',function(){
		  ...
		  var delstatus = $(this).attr("status");
		...
		swal({
          title: "Are you sure?",
          text: "All Data will be recoverd again!",
          type: "warning",
          showCancelButton: true,
          confirmButtonClass: 'btn-danger',
        ...
        function(isConfirm){
          if (isConfirm){
			...
			$.ajax({
				type: "post",
				url: 'show_func/'+delbtn,
				data: {id : delbtn},
				//contentType: "application/x-www-form-urlencoded",
				success: function(responseData) {
					//alert(responseData);
					//$('#row_'+delbtn).hide();
					swal("Recovered!", "Your data has been recovered!", "success");
					setTimeout(function () {
						location.reload();
					}, 2000);
				},
