...
$pdf->SetXY($current_x, $current_y);
	$pdf->MultiCell(2,10,'',0,0,L,1);
	$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
	$pdf->MultiCell(34,10,'Semester',0,0,L,1);
	$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
	$pdf->MultiCell(68,10,'Subject',0,0,L,1);
	...
	$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
	$pdf->MultiCell(34,10,'Semester',0,0,C,1);
	$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
	$pdf->MultiCell(75,10,'Subject',0,0,C,1);
}

if($institute_row['id']!='6bd9b29f-f4fd-c95f-466c-5afab5d415d5' && $institute_row['id']!='9710db7f-73f2-d3e0-6227-5afab568feab'){
	$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
	$pdf->MultiCell(30,10,'External Marks',0,0,L,1);
	$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
	$pdf->MultiCell(30,10,'Internal Marks',0,0,L,1);
}
else{
	//$pdf->Cell(20,10,'');
	//$pdf->Cell(20,10,'');
...
$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
$pdf->MultiCell(20,10,'Result',0,0,L,1);

if($institute_row['allow_grade']){
	$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
	$pdf->MultiCell(20,10,'Grade',0,0,L,1);
}
else{
	//$pdf->Cell(20,10,"",0,0);
}
...
if($institute_row['allow_score']){
	$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
	$pdf->MultiCell(20,10,'Total',0,0,L,1);
}
else {
	$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
	$pdf->MultiCell(20,10,'',0,0);
	//$pdf->Ln();
}
...
foreach($data as $k=>$marks_fix){
	...
	if($institute_row['id']!='6bd9b29f-f4fd-c95f-466c-5afab5d415d5' && $institute_row['id']!='9710db7f-73f2-d3e0-6227-5afab568feab'){
		$current_x+=$cell_width;                           //calculate position for next cell
	$pdf->SetXY($current_x, $current_y);
		$pdf->MultiCell($cell_width,$cell_height,'',0,0,L,0);
		$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
		$pdf->MultiCell($cell_width,$cell_height,"{$sem_name1[$k][0]}");
		$current_x+=$cell_width;                           //re-calculate position for next cell
		$pdf->SetXY($current_x, $current_y);
		$pdf->MultiCell($cell_width,$cell_height,"{$sub_name}");
	}
	else{
		$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
...
$pdf->SetXY($current_x, $current_y);
	...
	foreach($data_marks as $m_key=>$d_marks){
		if($m_key==$k){
			...
			if($institute_row['id']!='6bd9b29f-f4fd-c95f-466c-5afab5d415d5' && $institute_row['id']!='9710db7f-73f2-d3e0-6227-5afab568feab'){
				$current_x+=$cell_width;                           //calculate position for next cell
				$pdf->SetXY($current_x, $current_y);
				$pdf->MultiCell($cell_width,$cell_height,"{$externalmarks}",0,0,L);
				$current_x+=$cell_width;                           //calculate position for next cell
		$pdf->SetXY($current_x, $current_y);
				$pdf->MultiCell($cell_width,$cell_height,"{$internalmarks}",0,0,L);
			}
			else{
	...
	$pdf->MultiCell($cell_width,$cell_height,"{$status}");
	if($institute_row['allow_grade']){
		$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
		$pdf->MultiCell($cell_width,$cell_height,"{$grade}",0,0);
	}
	else{
		//$pdf->Cell(20,10,"",0,0);
	}
	if($institute_row['allow_score']){
		$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
		$pdf->MultiCell($cell_width,$cell_height,"{$overallmarks}",0,1);
	}
	else{
		$current_x+=$cell_width;                           //calculate position for next cell
$pdf->SetXY($current_x, $current_y);
		$pdf->MultiCell($cell_width,$cell_height,"",0,0);
		$pdf->Ln();
	}
