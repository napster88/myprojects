...
if ( is_wp_error( $user ) ) {
  ...
  $discountFor = $_POST['discount_for'];
    ...
    if($action == 'add'){
        ...
        if($userId) {
  	     ...
  	     /* Getting Product Details */
            ...
            $courseTotal     = get_product_price($course_id, $orderCurrency); 
            $orderinfo_user     = get_orderdetails_user($userId, $course_id);
            $order_total_amount = $orderinfo_user['total_course_amount'];
            ...
            $order_array = array(
              ...
              'total_course_amount' => $courseTotal,
          ...
          if(empty($orderinfo_user)){      
 
            add_user_meta($userId, 'order_' . $course_id . '', $order_array);
            add_post_meta($order->id, 'discount_for', esc_attr($discountFor));
            add_post_meta($order->id, 'discountamount', esc_attr($discount));
            add_user_meta($userId, 'order_discount_' . $course_id . '', esc_attr($discount));
            add_user_meta($userId, 'discount_for_' . $course_id . '', esc_attr($discountFor));
            add_user_meta($userId, 'order_' . $course_id . '_c', $orderCurrency);
            add_user_meta($userId, 'discount_for_order_' . $order->id . '', esc_attr($discount));  
          }  
        }
...
if($action == 'update'){ 
   ...
   //exit();
      ...
      if($discountFor){
        add_post_meta($orderId, 'discount_for', esc_attr($discountFor));
      }
      if($discount){
        add_post_meta($orderId, 'discountamount', esc_attr($discount));
      } 
      // if($orderTotal){
