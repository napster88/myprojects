...
class te_student_batchViewEdit extends ViewEdit {
	function display(){
		...
		$batchSql="SELECT `current_sems` as `sem_id`,`payment_plan` FROM `te_student_batch` WHERE id='".$record_id."'";
		...
		$.ajax({url: "index.php?entryPoint=paymentplandropdown&batchId="+$('#te_ba_batch_id_c').val()+"", success: function(result){ 
				...
				if(result.status=='ok'){			
				 ...
				 for(var i=0;i<result.res.length;i++){
						...
						if(id=="<?php echo $row_batch["payment_plan"];?>"){
							batch+='<option value="'+id+'"  selected="selected" >'+name+'</option>';
						}
						else{
							batch+='<option value="'+id+'">'+name+'</option>';
						}
...
$('#container_date_of_enrolment_trigger_c').on("hide", function() { 
    console.log("browser page has been hidden");
});

$("#detailpanel_2").click(function(){
	
	if($('#te_ba_batch_id_c').val()!=''){	
			$.ajax({url: "index.php?entryPoint=paymentplandropdown&batchIdenroll="+$('#te_ba_batch_id_c').val()+"", success: function(result){ 
							//$('#ajaxloading_c').hide(); //alert(result);return false;
							//SUGAR.ajaxUI.hideLoadingPanel();
				var result = JSON.parse(result);
				...
				if(result.status=='ok'){			
					 var batch='';
					 for(var i=0;i<result.res.length;i++){
							var duration = result.res[i].duration;
							var val=$("#date_of_enrolment").val();
			
							var newdate = new Date(val);

							newdate.setMonth(newdate.getMonth() + (duration*2));
							//newdate.setDate(newdate.getFullYear(),newdate.getMonth()+2,newdate.getDate()); // minus the date

							var nd = new Date(newdate);
							

							var  dateformat= nd.getFullYear();
							dateformat+="-";
							dateformat+=nd.getMonth()+1;
							dateformat+="-";
							dateformat+=nd.getDate();
							
							if(val){
								$("#course_expiry_date").val(dateformat);
							}
					 }
				}
			}
