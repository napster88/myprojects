...
class Favorites extends Favorites_sugar
{
    ...
    public function getCurrentUserSidebarFavorites($id = null)
    {
        ...
        while ($row = $db->fetchByAssoc($result)) {
            ...
            $return_array[$i]['item_summary'] = (!empty($bean->name)) ? $bean->name : '';//$bean->name;
            $return_array[$i]['item_summary_short'] = (!empty($bean->name)) ? to_html(getTrackerSubstring($bean->name)) : '';
            $return_array[$i]['id'] = $row['parent_id'];
            $return_array[$i]['module_name'] = $row['parent_type'];
            $return_array[$i]['image'] = (!empty($bean->name)) ? SugarThemeRegistry::current() ->getImage($row['parent_type'],'border="0" align="absmiddle"',null,null,'.gif',$bean->name) : '';
