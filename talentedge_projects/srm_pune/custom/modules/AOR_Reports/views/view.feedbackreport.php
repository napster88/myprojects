...
class AOR_ReportsViewFeedbackreport extends SugarView {
    ...
    $user_ids = "'" . implode("','", $uid) . "'";
		...
		if(isset($_POST['export']) && $_POST['export']=="Export"){
			$data="Student,Batch,Email,Phone,Feedback Given\n";
			$file = "feedback_report";
			$filename = $file . "_" . date ( "Y-m-d");
			foreach($councelorList as $key=>$councelor){
				if($councelor['feedback_given']==1){
					$feedback_given='Yes';
				}
				else if($councelor['feedback_given']==2){
					$feedback_given='Yes';
				}
				else{
					$feedback_given='NA';
				}
				$data.= "\"" . $councelor['student'] . "\",\"" . $councelor['batch'] . "\",\"". $councelor['email']. "\",\"". $councelor['mobile']. "\",\"". $feedback_given. "\"\n";
				//$data.= "\"" . $councelor['student'] . "\",\"" . $councelor['batch'] . "\",\"". $councelor['email']. "\"\n";
			}
			ob_end_clean();
...
#PS@Manish	
		...
		$sugarSmarty->display('custom/modules/AOR_Reports/tpls/feedback.tpl');
	}
}
?>
