...
<?php
  ...
  $postcheck=$post;
   ...
   $previousBatch=get_post_meta($batch_id,'product_parent',true);
   ?>
<?php if(isset($batch_id) && $batch_id != ''){
	$snippet_postid = $batch_id;
 }else{
	 $snippet_postid = get_the_ID();
 }
//echo "=======".$snippet_postid;
 if(get_field('schema_script', $snippet_postid, true) != NULL){
	 $snippet = get_field('schema_script', $snippet_postid, true);
	 $snippet = preg_replace('#</script>#is', '', preg_replace('#<script(.*?)>#is', '', $snippet));
	 echo '<script>'. $snippet .'</script>';
 } ?>
<div style="display:none" id="postid">
   <?php //echo $batch_id; echo "<pre>";print_r($post);echo "</pre>";?>
   <?php //echo get_post_meta($batch_id, '_regular_price', true); ?>
</div>
<link href="<?php echo esc_url(get_template_directory_uri()); ?>/css/course-detail.css?v=2.12" rel="stylesheet" />
<style>
...
<div style="display:none" id="singlealumni">
   <?php
      ...
      echo 'batch_code='. get_field('batch_name', $batch_id);
      ?>
</div>
<?php
...
<section id="post-<?php echo $batch_id; ?>" class="main-wrapper">
   <div class="te-banner-top" <?php echo $bannercolor; ?> <?php echo $bannerbg; ?>>
      <div class="container">
                     ...
                     <div class="institute-widget">
                        ...
                        <div class="instituteName">
                           <div class="middleAlign">
                              <h2>
                              <?php echo $coursearray['inst']['short_name']; ?></h2>
                           </div>
                        </div>
                     </div>
                  </a>
                  <?php
                     if ($coursearray['partner'])
                     {
                         $partner = $coursearray['partner'];
                         ?>
                  <a class="main_root" href="<?php echo get_permalink($partner); ?>">
                     <div class="institute-widget">
                        <picture><img src="<?php echo get_field('logo', $partner) ?>"/></picture>
                        <div class="instituteName">
                           <div class="middleAlign">
                              <h2><?php echo get_the_title($partner) ?></h2>
                           </div>
                        </div>
                     </div>
                  </a>
         ...
         <div class="right-te movable col-md-4 col-sm-4 xs-hidden">
            ...
            <div class="batch-Card">
               <?php
                  $exclusive = $coursearrayBatch['exclusive'];
                  if ($exclusive == 'Yes')
                  {
                      ?>
               <div class="exclusive">
               ...
               <div class="apply_btn" id="<?php echo $coursearrayBatch['select_course']; ?>">
                  ...
                  <div class="enrollCourse">
                     <a href="<?php echo home_url(); ?>/edit-profile#myCourses">
                        View Your Courses
                  </div>
                  <?php }
                     else
               ...
               <div class="coursePeriod">
                  <div class="months pad-bottom-10">
                     ...
                     <?php $duration_months = extract_numbers($coursearrayBatch['duration']);
                        ...
                        ?>
                      <span class="monthsOfCourse"><?=($batch_id==35006)? '2' : $duration_months[0];?></span>
                     <p><?=($batch_id==35006)? 'Days' : 'Months';?></p>
                  </div>
                  <div class="startDate pad-bottom-10">
                     ...
                     <?php $date            = new DateTime($cl_startdate); ?>
                       <span>Start - <?=($batch_id==35006)? '9 -10 Dec, 2017' : $date->format('jS M y');?></span>
                  </div>
                  <div class="timePeriod pad-bottom-10">
                     <div>
                        <i class="fa icon-clock"></i>
                        <span><?php echo get_field('schedule_of_classes',$batch_id) ?></span>
                     </div>
                  </div>
               </div>
            </div>
  ...
  <div class="cover_nav">
         ...
         <div class="container">
            <div class="valign">
               <ul id="list_scroll" class="nav">
                   <li><a href="#view_Course_Overview"><?=($batch_id==35006)? 'Conference Overview' : 'Course Overview'?></a></li>
                  ...
                  <li><a href="#view_Fee_EMI">
                     <?php echo get_field('fee_emi_headline'); ?></a>
                  </li>
                  <?php } ?>
   ...
   <?php
      if (have_rows('core_benefits',$batch_id)):
          while (have_rows('core_benefits',$batch_id)) : the_row();
              $ww = 1;
              ?>
   ...
   <?php echo have_rows('who_can_participate',$batch_id);
     // if (have_rows('who_can_participate',$batch_id)):echo "11222222222222";
          while (have_rows('who_can_participate',$batch_id)) : the_row();
              $ww = 1;//echo "test test";
              ?>
   <li>
   ...
   <h3 class="head-title col-md-12"><?php echo get_field('eligibility_headline',$batch_id); ?></h3>
   </div>
   <?php if (get_field('education',$batch_id))
      { ?>
   <div class="eligibil-left col-md-12 col-sm-12 col-xs-12">
   <h6>Education</h6>
   <h5><?php echo get_field('eligibility_subheadline',$batch_id); ?></h5>
   <div class="e_content"><?php echo get_field('education',$batch_id); ?></div>
   </div>
   <?php } ?>
   <?php if (get_field('work_experience',$batch_id))
      { ?>
   <div class="eligibil-left col-md-12 col-sm-12 col-xs-12">
   <h6><?php echo get_field('work_experience_headline',$batch_id); ?></h6>
   <div class="e_content"><?php echo get_field('work_experience',$batch_id); ?></div>
   </div>
   <?php } ?>
   </div>
   </div>
   ...
   <?php
      ...
      if ($posts):
          $feaculty_headline = get_field('feaculty_headline',$batch_id);
          ?>
   ...
   <img src="<?php echo $faculty_image; ?>" class="avatar">
   </div>
   <div class="left">
   ...
   <?php
      if (have_rows('certificates',$batch_id))
      {
          ?>
   <div class="left col-md-6 col-sm-6 col-xs-12 no-padding-left">
   <h3 class="head-title col-md-12">
   <?php echo get_field('certificate_headline',$batch_id); ?>
   </h3>
   <?php echo get_field('certificates_content',$batch_id); ?>
   </div>
   <div class="right col-md-6 col-sm-6 col-xs-12 pull-right no-padding-right">
   <div class="headCertificate"><a>View Sample Certificate</a></div>
   <aside class="list_certificates">
   <?php
      $cert = 1;
      while (have_rows('certificates',$batch_id)) : the_row();
   ...
   <?php
      ...
      ?>
       ...
       <div class="row">
           <div id="to_Why_Talentedge" class="row">

   <div class="flex-row">
       <div class="col-md-12 col-xs-12 no-padding">
   <?php if (have_rows('number_stats',$batch_id))
      { ?>
   ...
   <?php
      while (have_rows('number_stats',$batch_id)) : the_row();
          ?>
   <li>
   ...
   <?php if (have_rows('text_stats',$batch_id)): ?>
   <ul>
   <?php
      while (have_rows('text_stats',$batch_id)) : the_row();
          ?>
   ...
   <?php
    ...
    <?php if (!$hideForm && $batch_id!=35006)
      { ?>
   ...
   <i class="fa icon-clock"></i>
   <div>
   <?php echo get_field('schedule_of_classes',$batch_id) ?>
   </div>
   </div>
   <?php if ($brouchure)
      { ?>
   ...
   <?php
      $testimonials = get_field('testimonials', $batch_id);
            ...
            <div class="flex-row alumnis-widget">
               <div class="row">
                  <div class="learner-spearkers">
                     <?php if (get_field('testimonials_headline',$batch_id))
                        { ?>
                     <h2 class="alumnihead"><?php echo get_field('testimonials_headline',$batch_id); ?></h2>
                     <?php } ?>
      ...
      <div class="container">
         <div class="left-te col-md-8 col-sm-12 col-xs-12">
            <div class="flex-row alumnis-widget">
               <div class="row">
                  <div class="alumnidiv">
                     <?php if (get_field('alumni_headline',$batch_id))
                        { ?>
                     <h2 class="alumnihead"><?php echo get_field('alumni_headline',$batch_id); ?></h2>
                     <?php } ?>
                     <div class="clearfix">
   ...
   <div class="container">
      <div class="left-te col-md-8 col-sm-12 col-xs-12">
         <?php if (get_field('how_it_works_content',$batch_id))
            { ?>
         <div id="view_How_it_works">
            <div class="row">
               <?php if (get_field('how_it_works_headline',$batch_id))
                  { ?>
               <h2 class="head-title"><?php echo get_field('how_it_works_headline',$batch_id); ?></h2>
               <?php } ?>
               <div class="flex-row how_works">
                  <?php echo get_field('how_it_works_content',$batch_id); ?>
               </div>
            </div>
         </div>
         <?php } ?>
      ...
      <div class="container">
         <div class="row">
            <div class="col-md-8 col-sm-12 col-xs-12 institute_left">
               <div class="clearfix">
                  <h2><?php echo get_field('institute_headline',$batch_id) ?></h2>
               ...
               <div class="shortNotice_institute clearfix">
                  <h4>
                     <?php echo get_field('about_headline', $view_inst_id); ?>
                  </h4>
                  <?php echo get_field('about_info', $view_inst_id); ?>
                  <?php if (have_rows('stats', $view_inst_id)): ?>
                  <ul class="range_numaric">
                     <?php
      ...
      <div class="container">
         <div class="row">
            <div class="col-md-8 col-sm-12 col-xs-12 fee_emi">
               <?php
                  $s = 1;
                  if (have_rows('syllabus',$batch_id)):
                      ?>
               <div id="view_Syllabus" class="clearfix">
                  <h2><?php echo get_field('what_youll_learn_headline',$batch_id); ?></h2>
                  <div class="syntax-list">
                     <div class="panel-group" id="accordion">
                        <?php
                           while (have_rows('syllabus',$batch_id)) : the_row();
                               if ($s == 1)
                               {
                                   ?>
                        <div class="panel">
               ...
               <div id="view_Fee_EMI" class="clearfix">
                  <h2><?php echo get_field('fee_emi_headline',$batch_id); ?></h2>
                  <div class="syntax-list">
                                    ...
                                    <table class="table table-responsive tableFee">
                                       <tbody>
                                          <?php
                                             ...
                                             if ($i_price)
                                             {//echo "asdfadsfa".$batch_id;
$tax_free         = get_field('tax_free', $batch_id);
                                                 ?>
                                          <tr data="<?php echo $i_prices; ?>">
                                             <td>For Indian <?=($batch_id==35006)? 'Participants' : 'Residents';?> </td>
                                             <td align="right">
                                                <?php if ($discount_array['newprice_inr'])
                                                   { ?>
                                                <div class="tprice">
                                                   <h4 class="text-strike">Rs.<?php echo number_format($i_price); ?><?php if($tax_free=='Yes'){}else{?>+ GST*<?php }?></h4>
                                                   <h4><?php echo $discount_array['discount_title']; ?> Price:Rs. <?php echo number_format($discount_array['newprice_inr']); ?><?php if($tax_free=='Yes'){}else{?>+ GST*<?php }?></h4>
                                                </div>
                                                <?php }
                                                   else
                                                   { ?>
                                                <h4>Rs. <?php echo number_format($i_price); ?><?php if($tax_free=='Yes'){}else{?>+ GST*<?php }?></h4>
                                                <?php } ?>
                                                <?php
                                                   if ($new_date2)
                                                   {
                                                       ?>
                                                <?php
                                                   if ($coursearrayBatch['admission'] == 'No')
                                                   {
                                          ...
                                          <?php } ?>
                                          <?php
                                             $usd_price = get_post_meta($batch_id, '_outside-india_regular_price', true);
                                             if ($usd_price)
                                             {
                                                 ?>
                                          <tr>
                                             <td>For International <?=($batch_id==35006)? 'Participants' : 'Students';?></td>
                                             <td>
                                 ...
                                 <div class="emiStructure">
                                    <table class="table table-responsive emiSchedule">
                                       <?php if (have_rows('installments',$batch_id)): $im = 1; ?>
                                       <caption>Instalment Schedule</caption>
                                       <thead>
                                          <tr>
                                             <?php
                                                $im = 1;
                                                while (have_rows('installments',$batch_id)) : the_row();
                                       ...
                                       <?php
                                          $kk = 1;
                                          if (have_rows('installments',$batch_id)):
                                       ...
                                       <tbody>
                                          <tr>
                                             <td colspan="5" style="text-align:left;">
                                                <?php
                                                   //echo 'xx='. strlen($label1);
                                                   $label1 = get_field('label_1',$batch_id);
                                                   $label2 = get_field('label_2',$batch_id);
                                                   if ($kk == 1 && $label1)
                                                   {
                                                       ?>
                                          ...
                                          <tr>
                                             ...
                                             $ic=1;
                                                while (have_rows('installments',$batch_id)) : the_row();
                                                    ...
                                                    $cop           = get_sub_field('cop');
                                                    ?>
                                             <td>
                                                <div class="minstallment"><?php echo $imtext; ?></div>
                                                <?php
                                                   if ($discount_array['newprice_inr'] && $kk == $icount)
                                                   {
                                                       $disprice       = $discount_array['discount_price_inr'];
                                                       $new_i_discount = $in_currency - $disprice;
                                                       ?>
                                                <div class="amount"><span class="break_mb text-strike">Rs. <?php echo number_format($in_currency); ?> <?php if($tax_free=='Yes'){}else{?>+</br> GST*<?php }?></span></div>
                                                <div class="amount"><span class="break_mb">Rs. <?php echo number_format($new_i_discount); ?> <?php if($tax_free=='Yes'){}else{?>+</br> GST*<?php }?></span></div>
                                                <?php }
                                                   else
                                                   { ?>
                                                <div class="amount"><span class="break_mb">Rs. <?php echo number_format($in_currency); ?> <?php if($tax_free=='Yes'){}else{?>+</br> GST*<?php }?></span></div>
                                                <?php } ?>
                                                <?php
                                                   if ($discount_array['newprice_usd'] && $kk == $icount)
                                                   {
                                                ...
                                                <div style="">
                                                  <?php
                                                  $mmmm=   get_field('installments',$batch_id);
                                                  $cop   = get_sub_field('cop',$batch_id);
                                                  $check_advance='';

                                                  foreach($mmmm as $key=> $val)
                                                  {
                                                    if(is_array($val['cop']))
                                                    {
                                                      $check_advance=1;
                                                    ...
                                                    if ( $label2  && $check_advance==1)
                                                     {
                                                         ?>
                                                  <p class="pclass empty"><?php  if($ic==1) { echo $label2;  }
                            ...
                            <div class="amount"><span class="break_mb">Rs. <?php echo number_format($cop_inr_price); ?> +</br> GST*</span>
                            </div>
                                                        <?php } ?>
                            ...
                            <div class="amount"><span class="break_mb">USD <?php echo $cop_usd_price; ?></span>
                                ...
                                <?php if($tax_free!='Yes'){?> <span class="" style="font-weight: bold; display: none;">Effective 01 July 2017, the prevailing Service Tax will be replaced by GST at an effective rate of 18%</span><?php }?>
                              </div>
                           </div>
                        </div>
...
<?php
$termdata = get_term( $coursearray['cat'][0], 'course-categories' );
...
$coursearray          = $courses_arr2[$post->ID];
$prvlink=$_SERVER['HTTP_REFERER'];
$link=0;
if (preg_match('/search=/',$prvlink))
   $link=1;
//echo "<pre>";print_r($_SERVER);
 ?>
   <script>
       <?php if($batch_id==35006){ ?>
            $(".add_to_cart_button").text("Book your Seat");
            <?php } ?>
      $('.add_to_cart_button').click(function(){//alert('start');
        dataLayer.push({
  'event': 'addToCart',
  'ecommerce': {
    'currencyCode': 'INR',
...
        'name': "<?php echo $coursearray['name']; ?>",
        'id': '<?php echo $post->ID;?>',
        'price': "<?php echo $i_price;?>",
        'brand': "<?php echo $coursearray['inst']['short_name'];?>",
        'category': "<?php echo $termdata->name;?>",
        'variant': "<?php echo $new_date2; ?> Batch",
        'quantity': 1,
	'dimension6': "<?php echo $duration_months[0].' Months';?>" //3 months or 6 months etc.,
       }]
    }
  }
});
...
// Measure a view of course details
dataLayer.push({
  'event':'productDetail',
  'ecommerce': {
    'detail': {
      'actionField': {'list': '<?php echo $link==1?"Search Results":"Course Category";?>'},  // Here Business management can be replaced with from where we came to course detail page. For example, search results, Analytics etc.,
      'products': [{
        'name': "<?php echo $coursearray['name']; ?>",  // Name or ID is required.
        'id': '<?php echo $post->ID;?>',
        'price': '<?php echo $i_price;?>',
        'brand': "<?php echo $coursearray['inst']['short_name'];?>",
        'category': "<?php echo $termdata->name;?>",
...
<script>
   ...
   $('#input_5_18').val("<?php echo $brouchure; ?>");
   ...
   $('#input_9_14').val("<?php echo $coursearray['inst']['short_name']; ?>");
   $('#input_9_12').val("<?php echo $c_shortname; ?>");
   $('#input_9_10').val("<?php echo $batch_name; ?>");
   $('#input_9_5').val("<?php echo $brouchure; ?>");
   <?php if ($hideForm)
      { ?>
       $(document).ready(function () {
           ...
           function stickyRaightRail() {
               var summaries = $('.te-banner-top .right-te.movable');
           ...
           if ($('.te-banner-top .right-te.movable').length > 0) {
               stickyRaightRail();
           }
           $(window).resize();
       });
...
<script>
$(document).ready(function(){
   //$('#admission_batch').modal('show');
    });
</script>
 <?php } ?>
<!--<div id="myModal" class="modal fade" role="dialog" style="margin-top: 138px;">

  <div class="modal-dialog">
    <!-- Modal content-->
   <!-- <div class="modal-content">
      <div class="modal-header" style="background-color: #8adaec;">
        <!--<button type="button" class="close" data-dismiss="modal">&times;</button>-->
	<!--	<i style="cursor: pointer;position: absolute;right: -10px;top: -13px;border-radius: 50%;height: 25px;width: 25px;background: #ff0000;color: #fff;text-align: center;line-height: 24px;font-weight: normal;font-size: 16px;" data-dismiss="modal" class="fa fa-close userpro-button" id="close"></i>
        <h1 class="modal-title" style="text-align:center;font-size: 26px;">ADMISSION FOR THIS BATCH IS CLOSED</h1>
      </div>
      <div class="modal-body">
	   <div class="clearmar">
        <p style="text-align:center;font-size: 21px;">Do you want to check current open courses?</p>
		</div>
	   <div class="bd-example" data-example-id="" style="text-align: center;margin: 25px;">
      <button type="button" class="btn btn-md btn-primary" onclick="window.location.href='<?php echo esc_url( home_url( '/browse-courses/')); ?>';" id="open_course" style="margin-right: 39px;" >Yes<p>Show me open courses</p></button>
       <button type="button" class="btn btn-secondary btn-md" data-dismiss="modal" >No <p>I want to stay here</p></button>
       <div></div></div>
      </div>
...
<div id="admission_batch" class="modal fade" role="dialog">
  <div class="modal-dialog">

    <!-- Modal content-->
    <div class="modal-content">
        <button type="button" class="admission_close" data-dismiss="modal"></button>
      <div class="admission_batch_inner-container">
          <h1>Admissions for this batch is closed</h1>
          <h2>Do you want to check current open courses?</h2>
          <div class="popup_btn_container">
          <div class="col-md-6 col-sm-6 col-xs-12 no-padding-left">
          	<button class="confirm_btn" onclick="window.location.href='<?php echo esc_url( home_url( '/browse-courses/')); ?>';">
          		YES
				<span>Show me open courses</span>
			</button>
          </div>
          <div class="col-md-6 col-sm-6 col-xs-12">
          	<button class="reject_btn" data-dismiss="modal">
          		NO
				<span>I want to stay here</span>
			</button>
          </div>
