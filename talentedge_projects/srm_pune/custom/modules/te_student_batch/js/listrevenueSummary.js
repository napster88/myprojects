app.controller("revenueSummary", function($rootScope,$scope, $http) {
	...
	$scope.getResults=function(){
		...
		$http.post('index.php?module=te_student_batch&action=listrevenueSummary_ajax&to_pdf=1&emails='+ $scope.request.email +'&batches='+ $scope.request.batch+'&installment='+ $scope.request.installment+'&page='+$scope.page).success(function(data, status) {
			if($scope.page>1){
