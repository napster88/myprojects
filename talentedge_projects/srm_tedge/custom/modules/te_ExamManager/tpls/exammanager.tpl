...
<table cellpadding="0" cellspacing="0" width="100%" border="0" class="list view table footable-loaded footable default">
	...
	<tr height="20">
		...
		{if $form eq 002 && $slotcount eq 0 && $studentifoCount neq 0}	
			<strong><font size="3" color="red"><center>No Exam Slots For You!</center></font></strong>
		{/if}
		{if $studentifoCount eq 0}
		<strong><font size="3" color="red"><center>{$norecodmsg}</center></font></strong>
		{/if}	
{if $form eq 002 && $slotcount neq 0 && $studentifoCount neq 0}
	<!-- End Section student-->
<form name="search_form2" id="search_form2" class="search_form2" method="post" action="index.php?module=te_ExamManager&action=exammanager&form=004">
	...
	<tr height="20" class="oddListRowS1">
		<td align="left" valign="top" type="relate" field="total_leads" class="inlineEdit footable-visible footable-last-column">
		  <input type="checkbox" name="subjectid[{$i}]" value="{$key}" checked > {$subject_info.$key}
		</td>
		...
		<td align="left" valign="top" type="relate" field="total_leads" class="inlineEdit footable-visible footable-last-column">
			<input name="city[{$i}]" type="text"  value="" id='city' />
		</td>		
	</tr>
...
function getState(val) {
	$.ajax({
	type: "POST",
	url: "index.php?entryPoint=get_state.php",
	data:'country_id='+val,
