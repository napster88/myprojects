			...
			if(isset($_POST['button']) && $_POST['button']=="Submit" && $_REQUEST['form']==002){
					if($_POST['search_student']!=""){
							...
							while($Srow =$db->fetchByAssoc($studentObj)){ 
								  $studentifo[]=$Srow;
								  echo $currentsemID =$Srow['current_sems'];
								  $studentID =$Srow['studentid'];
								  $_SESSION['studentsessionid']=$studentID;	
							}
								  $_SESSION['registration_no']=$search_date;
								  $studentifoCount=count($studentifo);
							if($studentifoCount==0){
								
									$norecodmsg="No Record Found Or Enrollment ID Wrong !!";
								 }	 
					...
					if(!empty($studentifo)){
								...
								/*  Duplicate Exam booking fail */
							
							#$DupSql="SELECT id,exam_status,subject FROM `te_exammanager` WHERE deleted=0 AND te_student_id_c= '". $_SESSION['studentsessionid']."' ";
							$DupSql="SELECT exm.id,exm.exam_status,exm.subject,sem.name AS semname FROM `te_exammanager` AS exm INNER JOIN te_te_subject_te_te_semester_c AS semsubrel ON exm.subject=semsubrel.te_te_subject_te_te_semesterte_te_subject_idb INNER JOIN te_te_semester AS sem ON semsubrel.te_te_subject_te_te_semesterte_te_semester_ida=sem.id WHERE (exm.deleted=0) AND (exm.te_student_id_c='". $_SESSION['studentsessionid']."') AND (exm.exam_status IN ('Active','Pass'))";
						...
						if($BookedSubject){
							foreach($slotArr as $key=>$val){
								if(in_array($key,$BookedSubject)){
									unset($slotArr[$key]);
								}
							}
						}
					...
					if(isset($_POST['button']) && $_POST['button']=="Book-Exam" && $_REQUEST['form']==004){
							//echo '<pre>';
							//print_r($_POST);
							$sql = "INSERT INTO te_exammanager (id,subject,city,state,examnation_time,date_entered,te_student_id_c,exam_status,name) VALUES";
							$postvalus=array();
							...
							foreach($_POST['subjectid'] as $key=>$val){
							$postvalus[] = "('".create_guid()."','".$val."','".$_POST['city'][$key]."','".$_POST['state'][$key]."','".$_POST['date'][$key]."','".date('Y-m-d H:i:s')."','".$_SESSION['studentsessionid']."','Active','".$_SESSION['registration_no']."')";
							}			
							$sql .= join(',', $postvalus);
						...
						$sugarSmarty->assign("slotcount",$slotcount);
						$sugarSmarty->assign("studentifoCount",$studentifoCount);
						$sugarSmarty->assign("norecodmsg",$norecodmsg);
