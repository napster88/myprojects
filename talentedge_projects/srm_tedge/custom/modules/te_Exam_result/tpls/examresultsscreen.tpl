	...
	{assign var=i value=0}
		...
		<tr height="20" class="oddListRowS1">
			...
			<td align="left" valign="top" type="relate" field="total_leads" class="inlineEdit footable-visible footable-last-column">
					...
					<input type="hidden" value="{php }echo $exam_min_val; {/php}" name="examtype_passing_marks[{$examdata.subject}][{$examschme.name}]">
					<input type="hidden" value="{php }echo $exam_max_val; {/php}" name="examtype_total_marks[{$examdata.subject}][{$examschme.name}]">
					<input name="examtype_val[{$examdata.subject}][{$examschme.name}]" type="number" class="form-control" max="{php }echo $exam_max_val; {/php}" id="examtype{$examschme.name}{$i}" required >
					<span class="text-success"><strong>Max Marks:- {php }echo $exam_max_val; {/php} </strong></span>
					<span class="text-danger"><strong>Min Marks:-  {php }echo $exam_min_val; {/php}</strong></span>
