...
jQuery(document).ready(function() {
	...
	**/
	jQuery(document).on('submit', '.userpro form:not(.userpro-search-form)', function(e){
		...
		jQuery.ajax({
			url: userpro_ajax_url,
			data: form.serialize() + "&action=userpro_process_form&template="+form_data['template']+"&group="+form_data[ form_data['template'] + '_group' ]+"&up_username='+up_username,
			dataType: 'JSON',
			type: 'POST',
			error: function(xhr, status, error){
				userpro_end_load( form );
	...
	**/
	jQuery(document).on('blur', '.userpro[data-template=publish] textarea, .userpro[data-template=publish] input, .userpro[data-template=register] input, .userpro[data-template=edit] input, .userpro[data-template=change] input', function(e){
		...
		if (required == 1) {
            ...
            if ( jQuery(this).attr('data-label') == 'phone_number' ){ // only if field is password
                if ( !(phone_regex.test(jQuery(this).val()))) {
                    var finddiv = $(this).next().hasClass('userpro-warning');
                    if(!finddiv){    
                        jQuery(this).after("<div class='userpro-warning customWarning' style='opacity: 1; top: 0px;'>Please enter correct phone number.</div>")
                        jQuery('.userpro-icon-ok').hide();
                    }
            }
                else {
                     jQuery('.userpro-icon-ok').show();
                     $(this).next('.userpro-warning').remove();
                     $(this).removeClass('warning')
                    //userpro_client_valid(jQuery(this), jQuery(this).parents('.userpro-input'));
                }
            }
