		...
		function numbertype(&$bean, $event, $arguments) {
				...
				if(!empty($_REQUEST['te_in_institutes_id_c']) && !empty($bean->name) && $bean->is_active=='1')
				{
					/* Active Condition */
						...
						//print_r($progamID);
						//$prog=implode('^,^',$progamID);
						//echo $prog;
						//$bean->program_lising=$prog;
						//print_r($progamID);die;
						
						//echo $InsID;
						//print_r ($progamID); die("df");
						$EXamSql="SELECT program_lising FROM `te_exam_scheme` WHERE is_active='1' AND te_in_institutes_id_c ='".$InsID."' AND deleted=0"; 
