...
function leadAssignmentRuleJob()
{
	ini_set('display_errors', 0);
	ini_set("memory_limit","512M");
	global $db;
	$leadSql="SELECT l.id,l.lead_source,l.status,lc.te_ba_batch_id_c FROM leads l LEFT JOIN leads_cstm lc ON l.id=lc.id_c WHERE l.deleted=0  AND l.assigned_flag = 0 ORDER BY date_entered DESC LIMIT 0,50";
	//~ echo $leadSql."----";
	$leadObj = $db->query($leadSql);
	$flag=0;
	//~ echo "<pre>";
	while($row=$db->fetchByAssoc($leadObj)){
		$ruleSql="SELECT * FROM  te_lead_assignment_rule WHERE lead_source='".$row['lead_source']."' AND te_ba_batch_id_c='".$row['te_ba_batch_id_c']."' AND rule_status='active'";
		//~ echo "<br> Rule SQL ---".$ruleSql;
