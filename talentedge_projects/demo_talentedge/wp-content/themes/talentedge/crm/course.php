...
$_POST=$_REQUEST;
	...
	$program_name = $_POST['pname']; //NO
	$batchid =  $_POST['batchID'];
	$p_desc = $_POST['pexcerpt']; //NO
	$institute_crmid = $_POST['inst_crm_id'];
	$batch_start_date = $_POST['batch_start_date']; //NO
	$duration  = $_POST['duration']; //NO
	$fees_inr = $_POST['fees_inr']; //NO
	$fees_usd = $_POST['fees_usd']; //NO
	$course_type = $_POST['course_type']; //NO
	$course_status = $_POST['batch_status']; 
	$last_date_to_register = $_POST['last_date_to_register']; //NO
	$installmentinf = $_POST['installment_detail']; //NO
	$postStatus = $_POST['program_status'];
	$discountInr = $_POST['discount_inr']; //NO
	$discountUsd = $_POST['discount_usd']; //NO
...
//print_r($c);die;
   ...
   if($insID){
    ...
    //echo get_the_modified_time('h:i:sa', $course_id);
       ...
       $fp = fopen('course_api_log_.txt', 'a'); fwrite($fp,  "[".date("F d, Y h:i:s A")."] course_id updated=".$course_id); fclose($fp);
	    }
	    else{
	        $jsonvar[] = array('status' => false , 'message' => 'Not updated');
                
                
                 $fp = fopen('course_api_log_.txt', 'a'); fwrite($fp,  "[".date("F d, Y h:i:s A")."] course_id not updated=".$course_id); fclose($fp);
	    }
	}
echo json_encode($jsonvar); 
?>
