...
class relationship
{
    function maprelation($bean, $event, $argument)
    {
		...
		foreach($subject_master_map_data as $subject_map){
			$managekit="Select `id` from te_managekititem where kit_item_code='".$subject_map['subject_code']."' and deleted=0";
			...
			if($managekit_map->num_rows >0){
				...
				$insert_data="INSERT into `te_mapitemtodispatch` (`id`,`name`,`semester`,`institute_id`,`program_id`,`semester_id`,`te_te_semester_id`) VALUES ('".$dispatch_id."','".$subject_map['subjectname']."','".$subject_map['semestername']."','".$subject_map['semester_institute_id']."','','".$subject_map['semester_id']."','".$subject_map['semester_id']."')";
