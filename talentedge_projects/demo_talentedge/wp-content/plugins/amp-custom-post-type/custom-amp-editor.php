...
add_filter( 'amp_post_template_data', 'ampforwp_cpt_custom_post_content_sanitizer', 10, 2 );

function ampforwp_cpt_custom_post_content_sanitizer( $data, $post ) {
...
function ampforwp_cpt_custom_content_meta_register() {

  global $redux_builder_amp;

  if($redux_builder_amp['ampforwp-custom-type']){
    foreach($redux_builder_amp['ampforwp-custom-type'] as $custom_post){
          add_meta_box( 'custom_content_editor', esc_html__( 'Custom AMP Editor' ), 'ampforwp_cpt_content_editor_title_callback', $custom_post ,'normal', 'default' );
    }
  }

}
add_action('add_meta_boxes','ampforwp_cpt_custom_content_meta_register');


function ampforwp_cpt_content_editor_title_callback( $post ) {
...
function ampforwp_cpt_content_editor_meta_save ( $post_id ) {
  // Checks save status
...
add_action ( 'save_post' , 'ampforwp_cpt_content_editor_meta_save' );

// Save Rating Meta Field function
function ampforwp_cpt_checkbox_meta_save ( $post_id ) {
  // Checks save status
    $is_autosave    = wp_is_post_autosave( $post_id );
    $is_revision    = wp_is_post_revision( $post_id );
    $is_valid_nonce = ( isset( $_POST[ 'amp_content_editor_nonce' ] ) && wp_verify_nonce( $_POST[ 'amp_content_editor_nonce' ], basename( __FILE__ ) ) ) ? 'true' : 'false';
...
add_action ( 'save_post' , 'ampforwp_cpt_checkbox_meta_save' );
