...
class WC_Gateway_Payu_In extends WC_Payment_Gateway {
	...
	*/
    ...
    function generate_payu_in_form($order_id)
        ...
        $IDFCStatus = get_transient('IDFCStatus'); 
        $idfc_log=get_user_meta($user_id,'idfc_log',true);
        ...
        {
            //$payu_in_enabled = $this->settings['enabled'];
            //$this->merchantid = '355yxa';
            ///$this->salt = '2b5lchnL';
            $this->merchantid = '355yxa';
            $this->salt       = '2b5lchnL';


            //$usd_merchant = get_field('payu_inr_merchant_key','option');
            //$usd_salt = get_field('payu_inr_salt','option');
        }
        else
        {
            $usd_merchant     = get_field('payu_usd_merchant_key', 'option');
            $usd_salt         = get_field('payu_usd_salt', 'option');
            $this->merchantid = $usd_merchant;
            $this->salt       = $usd_salt;
            //$payu_in_enabled = 'no';
        } // End check currency
        //Hash data
        ...
        {
            $idfc_status = getIDFCLog($user_id, $product_id,$idfc_log);


           // if ($idfc_status == 1 && $IDFCStatus==1)
            if ($IDFCStatus==1)
            {
               
                $payu_in_args['offer_key']=OFFER_KEY;
	...
	function check_transaction_status() {
		...
		if($this->check_hash_after_transaction($salt, $txnRs)) {
			...
			if($txnRs['status']=='success'){
                                ...
                                $Order_id = explode(' ', $txnRs['productinfo']);
                                if($txnRs['productinfo']!='' && $Order_id[2]!=''){
                                ...
                                $idfc_log=get_user_meta($user_id,'idfc_log',true);
                                        ...
                                        //discount
                                if($txnRs['discount'] && $txnRs['discount']!=''){
                                     //$percentage = ($txnRs['offer']=='IDFC Debit Card Offer@2896')? 10 :'';
                                     $percentage = (isset($txnRs['offer_availed']))? 10 :'';
                                     add_post_meta( $Order_id[2], 'idfc_discount',$txnRs['discount'],true);
                                     add_post_meta( $Order_id[2], 'idfc_offer',$txnRs['offer'],true);
                                     add_post_meta( $Order_id[2], 'idfc_percentage',$percentage,true);
                                }

                                }
                                if($idfc_log==''){
                                    //echo 'uid='.$user_id.'--proID='.$product_id; die;
                                    add_user_meta( $user_id, 'idfc_log',$product_id,true);
                                } 
