...
class userpro_api {
 ...
 //                          else {
	...
	// 								$url = get_home_url().'?registered=1';
	// 								wp_redirect($url.'#loginpopup');	
	// 								exit;
	// 							if (userpro_get_option('google_signin_redirect')){
	// 								wp_safe_redirect( userpro_get_option('google_signin_redirect') );
	// 						}elseif(userpro_get_option('after_login') == 'no_redirect'){
 ...
 //                          else {
	...
	// 								$emailId = $user_info['email'];
	// 								//$displayName  = $user_info['displayName'];
	// 								$name = $user_info['name'];
	// 								$lastName = $name['familyName'];
	// 								$firstName = $name['givenName'];
	// 								$displayName  =  $firstName." ".$lastName;
	// 								$filteredArray = array("email"=>$emailId, "displayname"=>$displayName, "fname"=>$firstName,"lname"=>$lastName,"socialnetwork"=>"google" );
	// 								$queryData =  build_query($filteredArray);
	// 							  	$redirect_uri =$_SESSION['current_page_uri'];
	// 							    wp_safe_redirect( $redirect_uri."?".$queryData."#loginpopup");
	// 								 exit;
	// 							/*customization ends*/
...
function linkedin_authorize(){
		if(isset($_REQUEST['wplUsername']) && (isset( $_REQUEST['_wpnonce']) && wp_verify_nonce($_REQUEST['_wpnonce'] , 'linkedin_auth'))){
			...
			$firstName=$_REQUEST['wplFirstname'];	
			$lastName=$_REQUEST['wplLastname'];	
			$position=inputData($_REQUEST['wplPosition']);	
			$industry=inputData($_REQUEST['wplIndustry']);	
			$companyName=$_REQUEST['wplCompanyname'];
			$location=$_REQUEST['wplLocation'];
			$socialNetwork=$_REQUEST['wplSocialNetwork'];
			$linkedinCheck=$_REQUEST['wplLinkedinCheck'];				
			...
			if(!$user_id){
				...
				<script type="text/javascript">
				 ...
				 jQuery( document ).ready(function($) {
				 	$('#login_form, .userpro-field-billing_city, .userpro-field-user_designation, .userpro-field-user_industry').hide();
					$('.userpro-field-display_name input').val('<?php echo $displayName;?>');
					$('.userpro-field-user_login input').val('<?php echo $username;?>');
					$('.userpro-field-user_email input').val('<?php echo $email;?>');
					$('.userpro-field-usereditprofile_email input').val('<?php echo $email;?>');
					$('.userpro-field-user_li_linkedin_email input').val('<?php echo $email;?>');
					$('.userpro-field-first_name input').val('<?php echo $firstName;?>');
					$('.userpro-field-last_name input').val('<?php echo $lastName;?>');
					$('.userpro-field-profilepicture input').val('<?php echo $profileurl;?>');
					$('.userpro-field-billing_first_name input').val('<?php echo $firstName;?>');
					$('.userpro-field-billing_last_name input').val('<?php echo $lastName;?>');
					$('.userpro-field-billing_city input').val('<?php echo $location;?>');
					$('.userpro-field-user_li_position input').val('<?php echo $position;?>');
					$('.userpro-field-user_li_industry input').val('<?php echo $industry;?>');
					$('.userpro-field-user_socialnetwork input').val('<?php echo $socialNetwork;?>');
					$('.userpro-field-user_li_linkedincheck input').val('<?php echo $linkedinCheck;?>');
				 });	
				</script>	
				<?php 
				...
				$url = get_home_url().'?registered=1&registered_email='.$email;
				wp_redirect($url.'#loginpopup');
				}
			}
