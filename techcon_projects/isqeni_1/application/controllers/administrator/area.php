...
class Area extends CI_Controller {
   ...
   public function index()
   {
      ...
      if(isset($_POST) && count($_POST) > 0 && !isset($_POST['lang_submit']))
      {
        ...
        if($checks !== FALSE)
        {
           ...
           {
             if(! ($this->data->delete("area",$check)))
             {
      ...
      $data['data']        = $this->data->get("area",$data['langId']) ;
     ...
     $this->load->view('administrator/pages/area_table',$data);
   ...
   public function add()
   {
      ...
      {
        $data['data']          = $this->data->get("area",$data['langId'],array("id"=>$this->uri->segment(4))) ;
      ...
      $data['branchs'] = $this->data->get("branchs");
       $this->load->view('administrator/global/header');
       $this->load->view('administrator/global/topbar');
       $this->load->view('administrator/global/menu');
       $this->load->view('administrator/pages/area_form',$data);
       $this->load->view('administrator/global/footer');
   }
   ...
   public function delete($slug)
	{
		if($slug > 0)
		{
		  	$this->data->delete("area",$slug);
		}
    	redirect('administrator/area', 'refresh');
