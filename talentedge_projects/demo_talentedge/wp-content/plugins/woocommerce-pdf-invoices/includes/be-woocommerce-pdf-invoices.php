	...
	final class BE_WooCommerce_PDF_Invoices {
		...
		private function show_invoice_button( $title, $order_id, $wpi_action, $btn_title, $arr = array() ) {
			...
			echo $btn = '<a title="' . $title . '" href="' . $href . '" ' . $attr . '>' . $btn_title . '</a>';
                ...
                private function show_receipt_button( $title, $order_id, $wpi_action, $btn_title, $arr = array() ) {
			$title      = __( $title, 'woocommerce-pdf-receipt');
			$href       = admin_url() . 'admin-ajax.php?post=' . $order_id . '&action=genrate_receipt&bewpi_action=' . $wpi_action . '&nonce=' . wp_create_nonce( $wpi_action );
			$btn_title  = __( $btn_title, 'woocommerce-pdf-receipt');
			$attr       = '';
		...
		public function woocommerce_order_details_page_meta_box_create_invoice( $post ) {
			...
			if ( $invoice->exists() ) {
				...
				$this->show_invoice_button( __( 'Create invoice', 'woocommerce-pdf-invoices'), $post->ID, 'create', __( 'Create', 'woocommerce-pdf-invoices'), array( 'class="invoice-btn button grant_access"' ) );
                        ...
                        $this->show_receipt_button( __( 'Create receipt', 'woocommerce-pdf-receipt'), $post->ID, 'create', __( 'Receipt', 'woocommerce-pdf-invoices'), array( 'class="invoice-btn button grant_access"' ) );
		}
